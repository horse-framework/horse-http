...
namespace Horse.Mvc.Errors
{
    ...
    public class DevelopmentErrorHandler : IErrorHandler
    {
        ...
        public Task<IActionResult> Error(HttpRequest request, Exception ex)
        {
            ...
            builder.Append("<p style=\"font-size:12px; text-align:center; margin-top:20px; border-top:1px solid #dadada; padding:25px 10px 0px 10px; font-weight:400; color:#666\">Horse MVC Server</p>");
            builder.Append("</div>");
            builder.Append("</div>");
            builder.Append("</body>");
            builder.Append("</html>");
